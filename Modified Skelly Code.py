...
def Main():
    ...
    while again:
        ...
        again = PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber)
        ##

        # input()


def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):  # Added leaderboard
    Score = 0
    GameOver = False
    while not GameOver:
        ...
        # Spaces between the operators
        chars = []
        for item in UserInput:
            if item != " ":
                chars.append(item)
        UserInput = "".join(chars)
        ##
    ...
    # Leaderboard
    addToLeaderboard = input("Enter y to add your score to the leaderboard, anything else to continue. ")
    if addToLeaderboard == 'y':
        leaderboard(Score)

    # Play again
    playAgain = input("Enter y to play again, anything else to end.")
    if playAgain == 'y':
        print()
        print()
        return True
...
def leaderboard(Score):  # Leaderboard
    try:
        with open("leaderboard.txt", "r") as file:
            new = file.readlines()
            if Score > int(new[0]):
                print("Well done - you've achieved a new high score!")
                new.insert(0, str(Score))

            elif Score == int(new[0]):
                print("Well done! You've equalled the previous high score.")
                new.insert(0, str(Score))

            else:
                index = 0
                while index < len(new) and Score <= int(new[index]):
                    index += 1
                new.insert(index, str(Score))
        ...
        with open("leaderboard.txt", "w") as file:
            print("Since you're the first player, you automatically have the high score!")
...
def DisplayMovesLeft(Targets):  # Moves Left
    MovesLeft = 0
    while Targets[MovesLeft] == -1:
        MovesLeft += 1

    MovesLeft += 1
    print("You currently have " + str(MovesLeft) + " moves left.")
    print()
    print()
    ##


def DisplayLeaderboard():  # Displaying the leaderboard.
    with open("leaderboard.txt", "r") as file:
        new = file.readlines()
        for item in new:
            item = item.strip()
...
def ConvertToRPN(UserInput):  # Reverse Polish Notation
    Position = 0
    Precedence = {"+": 2, "-": 2, "*": 4, "/": 4, "^": 6}  # Added precedence for exponent.
    Operators = []
...
def EvaluateRPN(UserInputInRPN):
    ...
    while len(UserInputInRPN) > 0:
        while UserInputInRPN[0] not in ["+", "-", "*", "/", "^"]:  # Added exponent
        ...
        if UserInputInRPN[0] == "+":
            ...
            Result = Num1 / Num2
        elif UserInputInRPN[0] == "^":  # Added exponent.
            Result = Num1 ** Num2
        UserInputInRPN.pop(0)
        S.append(str(Result))
...
def CheckIfUserInputValid(UserInput):
    if re.search("^([0-9]+[\\+\\-\\*\\/\\^])+[0-9]+$", UserInput) is not None:  # Added exponent to ReGex.
        return True
    else:
        return False
